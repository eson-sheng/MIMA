$(function () {
    ...
    function index(json) {
        ...
        for (var k in json) {
            ...
            /*链接*/
            var link = data.link ? data.link : 'javascript:;';
            /*图标*/
            var logo = data.logo;
            /*备注*/
            var note = data.note;
            /*页面*/
            html += string_format(html_str, num, logo, appname, username, password, link, note);
        }
        /*赋值页面*/
        $("#main").html(html);
        /*不显示note备注提示*/
    ...
    function page_html_model() {
        var html_str =
            ...
            '<span><a href="{5}">{2}</a></span>' +
            '<p><i class="uname"></i>{3}</p>' +
            '<p><i class="passd"></i>{4}</p>' +
            '<p><i class="note"></i>{6}</p>' +
            '</li>';
        return html_str;
    }
    ...
    function action_edit() {
        $("#main > li > span:nth-child(2)").find('.edit').click(function () {
            ...
            var num = $(this).attr('num');
            /*名称*/
            var appname = $(this).parent().parent().find('a').html();
            /*账号*/
            ...
            var html_str = '<center><h1>修改密码数据：</h1>\n' +
                ...
                '<br>\n' +
                '<label for="link">链接：</label>\n' +
                '<input type="text" name="link" id="link" value="{4}"/>\n' +
                '<br>\n' +
                '<label for="logo">图标：</label>\n' +
                '<textarea name="logo" id="logo" cols="30" rows="10">{3}</textarea>\n' +
                '<br>\n' +
                '<label for="note">备注：</label>\n' +
                '<textarea name="note" id="note" cols="30" rows="10">{5}</textarea>\n' +
                '</form></center>';
            var html = string_format(html_str, appname, username, password, logo, link, note);
            layer.open({
                type: 1,
                title: false,
                shadeClose: true,
